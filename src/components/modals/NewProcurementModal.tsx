...
export const NewProcurementModal: React.FC<NewProcurementModalProps> = ({ isOpen, onClose }) => {
  const { createProcurementRequest, inventory } = useIOMS();
  ...
  const handleItemSelect = (name: string) => {
    setItemName(name);
    const matched = inventory.find((i) => i.name === name);
    if (matched) {
      ...
      <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 bg-indigo-800 text-white flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-bold">Form Permintaan Pengadaan Barang / Restock</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs">
          <div>
            ...
            >
              ...
              <option value="Material Lainnya / Khusus">Material Lainnya / Khusus</option>
            </select>
          </div>
          ...
          <div className="pt-2 flex items-center justify-end space-x-2">
            ...
            >
              ...
              <span>Kirim Permintaan ke Finance</span>
            </button>
          </div>
        </form>
      </div>
